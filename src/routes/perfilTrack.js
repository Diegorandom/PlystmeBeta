                                        ...
                                        objetosGlobales[0].session
                                            ...
                                            .then(function (track) {
                                                if (track.records.length >= 1) {
                                                    ...
                                                    objetosGlobales[0].session
                                                        .run('MATCH (a:artista {artistaId: {artistaId}}), (u:usuario {spotifyid:{userid}}) CREATE (a)-[:interpreta]->(a)<-[:Escuchado]-(u)', { artistaId: objetosGlobales[position].artist_data.id, userid: objetosGlobales[position].userid })
                                                        .then(function () {
                                                            console.log(records.name + ' Guardadx en la BD')


                                                            objetosGlobales[0].session
                                                                .run('MATCH (a:artista {artistaId: {artistaId}}) SET t.album={album}, t.nombre={nombre}, t.artistas={artistas}, t.duracion={duracion}, t.Contenido_explicito={Cont_explicito}, externalurls={externalurls}, href={href}, spotifyid={spotifyid}, popularidad={popularidad}, previewUrl={previewUrl}, uri={uri}, albumImagen={albumImagen}}) RETURN t', { album: records.album.name, nombre: records.name, artistas: artistas, duracion: records.duration_ms, Cont_explicito: records.explicit, externalurls: records.external_urls.spotify, href: records.href, spotifyid: records.id, popularidad: records.popularity, previewUrl: records.preview_url, uri: records.uri, albumImagen: records.album.images[2].url, artistaId: objetosGlobales[position].artist_data.id })
                                                                .then(function () {
                                                                    console.log('Se agregó información al nodo del artista')
                                                                })
                                                                .catch(function (error) {
                                                                    console.log(error);
                                                                })
                                        ...
                                        objetosGlobales[0].session
                                            ...
                                            .then(function (track) {
                                                if (track.records.length >= 1) {
                                                    ...
                                                    objetosGlobales[0].session
                                                        .run(
                                                            'MATCH (u:usuario {spotifyid:{userid}}),'
                                                            + '(n: artista { spotifyId: { artistaId } }) '
                                                            + 'SET n.external_urls: { external_urls }, n.seguidores: '
                                                            + '{ seguidores }, n.generos: { generos }, n.herf: { href }, '
                                                            + 'n.imagenes: { imagenes }, n.nombre: { nombre_artista }, '
                                                            + 'n.popularidad: { popularidad }, n.uri: { uri } }) - '
                                                            + '[: interpreta]-> (m: track { album: { album }, nombre: { nombre }, '
                                                            + 'artistas: { artistas }, duracion: { duracion },'
                                                            + ' Contenido_explicito: { Cont_explicito }, externalurls: { externalurls }, '
                                                            + 'href: { href }, spotifyid: { spotifyid }, popularidad: { popularidad }, '
                                                            + 'previewUrl: { previewUrl }, uri: { uri }, albumImagen: { albumImagen } })'
                                                            + '< -[: Escuchado] - (u)',
                                                            {
                                                                external_urls: objetosGlobales[position].artist_data.external_urls.spotify,
                                                                seguidores: objetosGlobales[position].artist_data.followers.total,
                                                                generos: objetosGlobales[position].artist_data.genres,
                                                                href: objetosGlobales[position].artist_data.href,
                                                                artistaId: objetosGlobales[position].artist_data.id,
                                                                imagenes: objetosGlobales[position].artist_data.images[0].url,
                                                                nombre_artista: objetosGlobales[position].artist_data.name,
                                                                popularidad: objetosGlobales[position].artist_data.popularity,
                                                                uri: objetosGlobales[position].artist_data.uri, album: records.album.name,
                                                                nombre: records.name, artistas: artistas, duracion: records.duration_ms,
                                                                Cont_explicito: records.explicit, externalurls: records.external_urls.spotify,
                                                                spotifyid: records.id, previewUrl: records.preview_url, albumImagen: records.album.images[2].url,
                                                                userid: objetosGlobales[position].userid
                                                            })
                                                        .then(function () {
