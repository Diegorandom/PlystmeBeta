...
var postPlaylist = require('../https/postPlaylist')
...
router.get('/create/playlist', function (req, res, error) {
  ...
  objetosGlobales[0].spotifyApi.setAccessToken(objetosGlobales[position].access_token);
  ...
  if (error == true || objetosGlobales == undefined || position == undefined) {
    ...
    if (objetosGlobales[position].playlist_id == undefined) {
      ...
      let response = postPlaylist(options);
